...
/obj/structure/necromorph/maw/user_buckle_mob(mob/living/M, mob/user, check_loc)
	if(issilicon(M))
		...
		return FALSE
	if(!do_after(user, M, 5 SECONDS, DO_IGNORE_HELD_ITEM|DO_PUBLIC, TRUE, CALLBACK(src, PROC_REF(buckle_mob_check), M)))
		return
	return ..()
...
/obj/structure/necromorph/maw/user_unbuckle_mob(mob/living/buckled_mob, mob/user)
	if(buckled_mob.buckled != src || !user.IsReachableBy(buckled_mob))
		return
	var/time_to_unbuckle = (buckled_mob == user) ? 8 SECONDS : 6 SECONDS
	if(!do_after(user, buckled_mob, time_to_unbuckle, DO_IGNORE_HELD_ITEM|DO_PUBLIC, TRUE, CALLBACK(src, PROC_REF(still_buckled), buckled_mob)))
		return
	return ..()
...
/obj/structure/necromorph/maw/relaymove(mob/living/user, direction)
	if(SEND_SIGNAL(src, COMSIG_ATOM_RELAYMOVE, user, direction) & COMSIG_BLOCK_RELAYMOVE)
		return
	if(!do_after(user, src, 2 SECONDS, DO_IGNORE_HELD_ITEM|DO_PUBLIC) || (user.buckled != src))
		return
